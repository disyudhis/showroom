...
new class extends Component {
    ...
    {
        return [
            'cars' => Cars::when($this->search, function ($query) {
                return $query->where(function ($q) {
                    $q->where('nama_mobil', 'like', '%' . $this->search . '%')
                        ->orWhere('brand', 'like', '%' . $this->search . '%')
                        ->orWhere('no_polisi', 'like', '%' . $this->search . '%')
...
<div class="space-y-6">
    ...
    <!-- Car Grid -->
    <div
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 bg-white dark:bg-gray-900 transition-colors duration-300">
        @forelse($cars as $car)
            <div
                class="bg-white dark:bg-gray-800 shadow-lg rounded-xl overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl group">
                @php
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex justify-between items-center mt-4">
                        <a href="{{ route('dashboard.show', ['car' => $car->id]) }}" wire:navigate
